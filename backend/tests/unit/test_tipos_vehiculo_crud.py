"""Schema-only tests for ``prod.tipos_vehiculo`` Pydantic schemas (PR3).
...
import pytest
from parkos_core.schemas.tipos_vehiculo import (
    TiposVehiculoCreate,
    TiposVehiculoFilter,
    TiposVehiculoRead,
...
class TestConfigForbid:
    ...
    def test_create_rejects_unknown_field(self):
        with pytest.raises(ValidationError):
            TiposVehiculoCreate(tipo="carro", rogue_field="evil")

    def test_update_rejects_unknown_field(self):
        with pytest.raises(ValidationError):
            TiposVehiculoUpdate(tipo="carro", rogue_field="evil")
...
class TestVersioningColumnsExcluded:
    ...
    def test_create_rejects_versioning_column(self, forbidden):
        with pytest.raises(ValidationError):
            TiposVehiculoCreate(tipo="carro", **{forbidden: "x"})

    @pytest.mark.parametrize("forbidden", ["vigente_desde", "vigente_hasta", "estado"])
    def test_update_rejects_versioning_column(self, forbidden):
        with pytest.raises(ValidationError):
            TiposVehiculoUpdate(tipo="carro", **{forbidden: "x"})
...
class TestFilterAllOptional:
    ...
    def test_populated_filter(self):
        f = TiposVehiculoFilter(estado="activo", vigente_desde__gte=datetime(2026, 1, 1))
...
class TestReadList:
    def test_empty_list(self):
        rl = TiposVehiculoReadList(items=[], next_cursor=None)
        assert rl.items == []
    ...
    def test_with_cursor(self):
        ...
        rl = TiposVehiculoReadList(items=[r], next_cursor="abc")
