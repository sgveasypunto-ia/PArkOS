...
Coverage — the per-audit-class dispatch table:
  ...
  - ``[V]`` with ``natural_key`` (``clientes``) -> delegates to
    ...
    ``APPLIED``.
  - ``[V]`` without ``natural_key`` (``empresa``) -> seq comparison via
    ``ReadLocalSeq`` (D4) -> ``APPLIED`` / ``MANUAL(seq_tiebreak)`` with a
    ``sync_conflict`` row written.
...
async def test_v_without_natural_key_manual_when_remote_seq_missing(
    pg_session, make_spec
) -> None:
    """The remote payload has no comparable field -> can't prove newer -> MANUAL."""
    spec = make_spec("empresa")
    row_uuid = uuid_lib.uuid4()
    pg_session.add(Empresa(uuid=row_uuid, created_at=_naive(datetime(2026, 1, 1, tzinfo=UTC))))
    await pg_session.flush()
