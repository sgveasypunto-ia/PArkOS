"""test_dispatcher_hash_chain_branch_not_cloud.py â€” T-PR11-12, REQ-X4.
...
``repo/hash_chain.py::_read_head``):
  ...
  3. :func:`repo.hash_chain._read_head` queries prior chain head
     by ``model_cls.uuid_sucursal == uuid_sucursal`` â€” different
     branches therefore have different chain heads.

This test verifies both halves of the contract end-to-end on the
cloud-router flow. No real DB â€” all session interactions are mocked.
"""
...
from parkos_core.repo.hash_chain import _genesis_hash, _read_head


async def test_record_event_stamps_branch_uuid_and_chain_is_per_sucursal() -> None:
    """T-PR11-12 â€” log_transaccional stamped with branch.uuid, chain is per-sucursal."""
    ...
    # _read_head bootstraps a real genesis row (_ensure_genesis_row),
    ...
    # log row record_event writes â€” filter it out by accion.
    log_rows = [
        o for o in added if isinstance(o, LogTransaccional) and o.accion != "inicialización"
    ]
    ...
    # --- Part 2: _read_head returns the BRANCH's prior hash_actual ---
    ...
    # 0058: the head's chain position. _read_head returns head.seq + 1 as the
    # next row's seq, so a plain MagicMock here would break the arithmetic.
    branch_prior_row.seq = 7
    branch_prior_row.timestamp_evento = datetime(2026, 1, 1, tzinfo=UTC).replace(tzinfo=None)
    ...
    branch_seq, prior_hash_for_branch = await _read_head(
    ...
    assert branch_seq == 8, f"next seq must be head.seq + 1; got {branch_seq!r}, expected 8"
    ...
    cloud_seq, prior_hash_for_cloud = await _read_head(cloud_session, LogTransaccional, cloud_uuid)
    ...
    assert cloud_seq == 2, (
        f"an empty chain bootstraps genesis at seq 1, so the next row is 2; got {cloud_seq!r}"
    )
    assert prior_hash_for_cloud != prior_hash_for_branch, (
        "cloud's chain head MUST differ from the branch's â€” "
