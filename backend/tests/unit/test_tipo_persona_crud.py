"""Schema-only tests for ``prod.tipo_persona`` Pydantic schemas (PR3).
...
import pytest
from parkos_core.schemas.tipo_persona import (
    TipoPersonaCreate,
    TipoPersonaFilter,
    TipoPersonaRead,
    TipoPersonaReadList,
...
class TestConfigForbid:
    ...
    def test_create_rejects_unknown_field(self):
        with pytest.raises(ValidationError):
            TipoPersonaCreate(tipo="natural", rogue_field="evil")

    def test_update_rejects_unknown_field(self):
        with pytest.raises(ValidationError):
            TipoPersonaUpdate(tipo="natural", rogue_field="evil")
...
class TestVersioningColumnsExcluded:
    ...
    def test_create_rejects_versioning_column(self, forbidden):
        with pytest.raises(ValidationError):
            TipoPersonaCreate(tipo="natural", **{forbidden: "x"})

    @pytest.mark.parametrize("forbidden", ["vigente_desde", "vigente_hasta", "estado"])
    def test_update_rejects_versioning_column(self, forbidden):
        with pytest.raises(ValidationError):
            TipoPersonaUpdate(tipo="natural", **{forbidden: "x"})
...
class TestTipoLength:
    ...
    def test_create_rejects_bad_length(self, bad):
        with pytest.raises(ValidationError):
            TipoPersonaCreate(tipo=bad)

    def test_create_accepts_min_length(self):
        c = TipoPersonaCreate(tipo="x")
        assert c.tipo == "x"

    def test_create_accepts_max_length(self):
        c = TipoPersonaCreate(tipo="x" * 64)
...
class TestFilterAllOptional:
    ...
    def test_populated_filter(self):
        f = TipoPersonaFilter(estado="activo", vigente_desde__gte=datetime(2026, 1, 1))
...
class TestReadList:
    def test_empty_list(self):
        rl = TipoPersonaReadList(items=[], next_cursor=None)
        assert rl.items == []
    ...
    def test_with_cursor(self):
        ...
        rl = TipoPersonaReadList(items=[r], next_cursor="abc")
