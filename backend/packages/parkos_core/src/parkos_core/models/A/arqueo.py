"""ORM model for ``prod.arqueo`` (cash-count snapshot, [A] table, REQ-26-A-ARQUEO).

An ``arqueo`` row records a cash-count event: expected vs reported
balances per payment method. Like ``caja``, it's INSERT-only — corrections
are compensating rows.
...
class Arqueo(AppendOnlyBase):
    """[A] Cash-count event. INSERT-only; corrections are compensating rows."""
    ...
    uuid_tipo_arqueo: Mapped[uuid_lib.UUID | None] = mapped_column(
        PG_UUID(as_uuid=True),
        nullable=True,
    )
    ...
    valor_efectivo_esperado: Mapped[Decimal | None] = mapped_column(
        Numeric(precision=18, scale=4),
        nullable=True,
    )
    valor_datafono_esperado: Mapped[Decimal | None] = mapped_column(
        Numeric(precision=18, scale=4),
        nullable=True,
    )
    valor_efectivo_reportado: Mapped[Decimal | None] = mapped_column(
        Numeric(precision=18, scale=4),
        nullable=True,
    )
    valor_datafono_reportado: Mapped[Decimal | None] = mapped_column(
        Numeric(precision=18, scale=4),
        nullable=True,
    )
