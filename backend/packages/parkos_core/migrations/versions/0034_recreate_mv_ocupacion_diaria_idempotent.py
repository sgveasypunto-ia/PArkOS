...
revision table already marks 0024 as applied. This migration:
  ...
  1. ``CREATE MATERIALIZED VIEW IF NOT EXISTS prod.mv_ocupacion_diaria``
     using the canonical SELECT (identical body to 0024). PG does NOT
     support ``CREATE OR REPLACE MATERIALIZED VIEW`` (verified 2026-09-17
     against live ``PostgreSQL 16.15`` — the syntax fails with
     ``syntax error at or near "MATERIALIZED"``); the only idempotent
     pattern that works on PG is ``IF NOT EXISTS`` plus a separate
     re-assertion block (steps 3+4 below).
  2. Post-upgrade ``to_regclass`` assertion — fails the migration with
     ...
     dropping ``IF NOT EXISTS`` support for materialized views).
  3. Recreates the UNIQUE INDEX required by REFRESH CONCURRENTLY if it
     ...
     ``IF NOT EXISTS`` keeps the second-run case idempotent. Plain
     ``CREATE UNIQUE INDEX`` (no CONCURRENTLY) — alembic runs this
     migration inside a transaction block, and CONCURRENTLY is invalid
     in transactions.
...
``CREATE MATERIALIZED VIEW IF NOT EXISTS`` form is the only DDL shape
that is safe against both the missing-MV case AND a previously-applied-
0024-with-MV case on PostgreSQL 16.
...
down_revision = "0033_login_historic_index"  # rebased: chain must terminate on current head (verify 2026-09-17)
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    # 1) Create the MV if missing. ``CREATE MATERIALIZED VIEW IF NOT
    #    EXISTS`` is idempotent against both missing-MV and
    #    present-MV states. (Note: PG does NOT support ``CREATE OR
    #    REPLACE MATERIALIZED VIEW`` — verified live 2026-09-17.) The
    #    SELECT is identical to 0024 body so the contract
    #    (uuid_sucursal, uuid_tipo_vehiculo, activos) is preserved
    #    byte-for-byte.
    op.execute(
        """
    ...
    #    relation does not exist; if ``CREATE MATERIALIZED VIEW IF NOT
    #    EXISTS`` was somehow a no-op (future Postgres dropping the
    #    ``IF NOT EXISTS`` syntax), this catches it loudly rather than
    #    silently shipping an MV-less release.
    op.execute(
        """
        DO $$
        BEGIN
            IF to_regclass('prod.mv_ocupacion_diaria') IS NULL THEN
                RAISE EXCEPTION 'mv_ocupacion_diaria_missing_post_create: '
                                'CREATE MATERIALIZED VIEW did not produce the '
                                'view. Inspect the migration log.';
            END IF;
        END $$;
        """
    )
    ...
    #    present (the 0024 grant preserved it). Plain CREATE INDEX
    #    (no CONCURRENTLY) because alembic runs this migration inside
    #    a transaction block and CONCURRENTLY is invalid there. NOTE:
    #    ``IF NOT EXISTS`` does NOT support schema-qualified index
    #    names on PostgreSQL 16 (verified live 2026-09-17 against PG
    #    16.15 — fails with ``syntax error at or near "."``), so the
    #    index name is bare (the table's schema places it in ``prod``
    #    automatically).
    op.execute(
        """
