...
All tasks implemented and merged into `dev` per git log (commit `{{SHA}}` — see
...
for the operator (login → abrir turno → cotizar → registrar salida → PagoSheet
→ FE status → reimprimir tiquete → cerrar turno) with all checks green.
...
- `prod.alerta.datos_nuevos` column missing on live branch-db blocks
  ...
  `ALTER TABLE prod.alerta ADD COLUMN IF NOT EXISTS datos_nuevos jsonb` —
...
- Phase implementation commit: **`{{SHA}}`** (see `git log --grep`)
...
```bash
git mv openspec/changes/archive/2026-09-17-{{NAME}} openspec/changes/{{NAME}}
