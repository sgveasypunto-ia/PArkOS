...
- Substrate files (verified paths):
  ...
  - `pending-fase-10.md` (item #4: ABBC-F10.2-BE-1, preserved;
    ...
    reconciliation)
# Delta Spec: `operations` â€” Fase 11.1 Sync Banner
...
| Phase | Fase 11 â€” Operational telemetry and alerts (HU-F11.1 of 2) |
...
| Status | DELTA â€” 7 ADDED requirements (REQ-OPS-170..176). |

## ADDED Requirements

### REQ-OPS-170 â€” `useSyncEstado` Zod schema matches backend `SyncEstadoRead` (DA-F11.1-7 GATING)
...
### REQ-OPS-171 â€” `<SyncBanner />` top-of-page color strip with derived state
...
- `never_synced` â€” `ultima_sync_at IS NULL` (distinct NEUTRAL badge, NOT red).
- `online` â€” `lag_seg < 300` AND `ultima_sync_at` within last 3 600 s AND `pendientes < 100`.
- `lagging` â€” `300 <= lag_seg < 3 600` OR `1 <= pendientes < 100`.
- `offline` â€” `lag_seg >= 3 600` OR `ultima_sync_at` older than 3 600 s OR `pendientes >= 100`.

The component MUST consume `useSyncEstado` (SWR `refreshInterval: 30_000`), set `role="status"` + `aria-live="polite"` + `aria-atomic="true"`, and announce ONLY on state transition (last-announced-state ref + 2 s debounce â€” copy verbatim the F2.3 `StatusBar.tsx` lines 90-100 pattern).
...
- WHEN `<SyncBanner />` renders
- THEN it MUST render the NEUTRAL "Sin sincronizaciones aun" badge â€” NOT red â€” to avoid alarm fatigue on fresh installs.

### REQ-OPS-172 â€” `<LocalApiDownBanner />` separate component on 3 consecutive API failures

A SEPARATE component `<LocalApiDownBanner />` MUST render when `apiStatusStore.consecutiveFailures >= 3` and MUST NOT render otherwise. The component MUST use `role="alert"` + `aria-live="assertive"` (hard fault), with `aria-label="Estado de API local"` and copy "Sin conexion con API local" â€” distinct from `<SyncBanner />`. It MUST NOT share i18n keys with `<SyncBanner />` (DA-F11.1-1 + DA-F11.1-6).
...
### REQ-OPS-174 â€” Global mount in `App.tsx`

`<SyncBanner />` and `<LocalApiDownBanner />` MUST be mounted in `apps/electron-sucursal/src/renderer/App.tsx` adjacent to the existing `<StatusBar />` (DA-F11.1-1 â€” NOT replacing it). Order: `<StatusBar />` first (its API chip stays), then `<SyncBanner />`, then `<LocalApiDownBanner />`. Both new banners MUST render on every protected route.
...
### REQ-OPS-175 â€” `e2e/sync-banner.spec.ts` with 4 scenarios + axe-core gate
...
1. **(a) verde** â€” `lag_seg` bajo, sync dentro ultima hora.
2. **(b) amarillo** â€” `lag_seg` por encima del umbral, sin fallos consecutivos.
3. **(c) rojo** â€” sync fallida o `lag_seg > 3600` OR `consecutiveFailures >= 3`.
4. **(d) LocalApiDownBanner** â€” `consecutiveFailures === 3`, banner visible con copy distinto.

Each scenario MUST call `page.clock.install({ time: 0 })` + `page.clock.fastForward(30_000)` for SWR poll advancement (Playwright 1.45+; DA-F11.1-5 â€” MUST NOT use `test.skip`). The spec MUST include an axe-core WCAG 2.1 AA gate at the end (RNF-022).
...
### REQ-OPS-176 â€” RED tests for `useSyncEstado` + `SyncStatusStrip` close the untested-substrate gap (DA-F11.1-8)

Unit tests MUST land BEFORE any source change in T1 of apply (strict_tdd):
- `useSyncEstado.test.ts` â€” Zod parse against the real backend `SyncEstadoRead` shape (REQ-OPS-170), 401 â†’ auth cleared, 403/404 â†’ no retry.
- `SyncStatusStrip.test.tsx` â€” color mapping per state, never-synced badge, aria-live announce-on-transition only.
- `apiStatusStore.test.ts` â€” counter increment/reset, threshold derivation (REQ-OPS-173).
...
|---|---|---|
| DA-F11.1-1 (HIGH â€” StatusBar overlap) | RESOLVED | Distinct copy ("Sin conexion con API local") + distinct `role` (`alert` vs StatusBar `status`) + distinct `aria-label`. F11.1 does NOT replace StatusBar; renders SEPARATE banner (REQ-OPS-172). |
| DA-F11.1-2 (MED â€” dual read) | RESOLVED | StatusBar re-routed through `apiStatusStore` (single source of truth) per REQ-OPS-173. |
| DA-F11.1-3 (MED â€” threshold storage) | RESOLVED | Zustand `consecutiveFailures` counter (REQ-OPS-173), explicit constant `LOCAL_API_DOWN_THRESHOLD = 3`. |
| DA-F11.1-4 (LOW â€” aria-live spam) | RESOLVED | Verbatim F2.3 StatusBar.tsx L90-100 pattern (lastAnnouncedState ref + 2 s debounce), codified in REQ-OPS-171. |
| DA-F11.1-5 (MED â€” 30 s poll test) | RESOLVED | `page.clock.install` + `fastForward(30_000)` per REQ-OPS-175; no `test.skip` allowed (strict_tdd). |
| DA-F11.1-6 (HIGH â€” distinct banners) | RESOLVED | 2 SEPARATE components with distinct `role`, `aria-label`, i18n keys (REQ-OPS-171 + REQ-OPS-172). |
| DA-F11.1-7 (HIGH GATING â€” schema drift) | RESOLVED | Backend `SyncEstadoRead` (sync_infra.py:399) has `ultima_sync_at`, `lag_seg`, `pendientes` ONLY â€” NO `estado`, NO `ultimo_error`, NO `ultima_sync`. **FE is wrong on 4 axes** (extra fields + name mismatch + nullability). F11.1 fixes FE; no BE delta needed. Codified in REQ-OPS-170. |
| DA-F11.1-8 (MED â€” untested substrate) | RESOLVED | RED tests land BEFORE any source change (REQ-OPS-176). |
...
- **R-RES-2** (LOW): `<StatusBar />` re-routing through `apiStatusStore` is in-scope for this HU but is a F2.3 refactor. Mitigation: ship the store first (T1 of apply), rewire StatusBar second (T5) â€” both in same PR.
- **R-RES-3** (LOW): `parkosFetch` 401 path already clears `useAuthStore` (verified in `useSyncEstado.ts:60-66`). F11.1 inherits â€” no new logic.
...
## Phase 26 — Fase 11 frontend deltas — HU-F11.2 AlertasPanel (2026-09-21)
...
The query parameter `estado` MUST be exactly `activa`. The legacy vocabularies `abierta` / `cerrada` are NOT permitted at any layer (F11.1 stub is deleted). The endpoint MUST NOT include `severidad`, `descripcion`, or `mensaje` (those live on `alert_types` and are merged client-side per REQ-OPS-179).
...
| `tipo_alerta` (subset) | Drill-down route | Reference field |
|---|---|---|
...
`apps/electron-sucursal/src/features/alertas/hooks/useAlertas.ts` MUST be promoted from the F11.1 stub. The hook MUST:
...
DELETE: `useAlertas`, `fetchAlertas`, `AlertaSchema`, `AlertaArraySchema`, `Alerta` type from F11.1 stub. Replace single import in `AlertasPanel.tsx` with `useAlertas` from the new module path.
...
`AlertaSchema` (the FE schema that parses `GET /workflows/alerta` rows) MUST declare exactly the 18 fields from REQ-OPS-177, with the `estado` field constrained to `z.enum(['activa', 'descartada', 'resuelta'])` (canonical BE `Literal`, DA-F11.2-9 reconciled). The schema MUST reject any other field name at parse time (defense in depth against backend schema drift; precedent in F11.1 REQ-OPS-170).

The schema MUST additionally declare `datos_nuevos: z.record(z.unknown()).nullable().optional()` — this field is NOT exposed by `AlertaRead` today (DA-F11.2-14 is speculative until F11.2 lands); the field MUST be `.optional()` so the schema succeeds against the current BE response and gracefully picks up the column once the JOIN lands (ABBC-F11.2-BE-1 path a) OR once `datos_nuevos` is added to the canonical `AlertaRead`.
...
3. On `403 Forbidden` with body `{"error": "actor_is_target"}`: surface a typed toast (descartada actor-check; per REQ-26-W-ALERTA-DESCARTADA this endpoint will reject when `uuid_usuario == ctx.actor_uuid`. For `resuelta` this MUST NOT trip the check).
...
- AND the next render of `<AlertasPanel />` MUST NOT include the original alert `A`.
...
`apps/electron-sucursal/src/lib/alertas/constants.ts` MUST export `BUSINESS_ALERT_CODES: Set<string>` (11 business alerts) and `TECHNICAL_ALERT_CODES: Set<string>` (8 technical alerts — `hash_chain_anomaly`, `dian_rechazada`, `dian_timeout`, `dian_error`, `branch_offline_reauth_required`, `orphan_workflow_chain`, `fe_provider_error`, `fe_numbering_exhausted`). The exact 11-code whitelist MUST be derived from the operational alerts surfaced in `plan.md:2303-2344`.

`useAlertas` MUST drop any row whose `tipo_alerta` is in `TECHNICAL_ALERT_CODES` (or not in `BUSINESS_ALERT_CODES`) before populating `mergedAlertas`. The drop MUST NOT throw, MUST NOT log at `error` level, and MUST emit exactly one `console.debug` per dropped code (per ABIERTO-06).
...
## Phase 27 — Fase 12 frontend + backend deltas — HU-F12.1 Panel Mi turno (2026-09-21)

# Delta Spec: `operations` — Fase 12.1 Mi turno
...
| Phase | Fase 12 — Local minimal reporting (HU-F12.1 of 1) |
| Base spec | `openspec/specs/operations/spec.md` (last REQ-OPS-183 from F11.2) |
| Gap | No per-turn live KPI; F10.2 only aggregates at `cierre_turno` time |
| Preflight | pace=auto, artifact=hybrid (OpenSpec + Engram), delivery=auto-chain, budget=2000 LOC (Fase 12 meta-budget), strict_tdd=true, test=vitest + pytest + playwright, author=`Parkos Dev <dev@parkos.local>` |
| Status | DELTA — 7 ADDED requirements (REQ-OPS-184..190). 10 drift anchors resolved in this spec (DA-F12.1-10 GATING re. temporal JOIN via `[L-E]`/`[A]` immutability). |

## ADDED Requirements

### REQ-OPS-184 — `GET /operacion/mi-turno?uuid_sesion=X` response shape
...
- GIVEN an open `Sesion` X with no `ingreso` / `salidas` / `factura_pagos` events in its window
- WHEN client sends `GET /operacion/mi-turno?uuid_sesion=X`
- THEN response is 200 with all count/decimal fields equal to `0` (NOT 404, NOT error).

### REQ-OPS-185 — Tenant pin enforcement (DA-F12.1-2)
...
### REQ-OPS-186 — SQL aggregation strategy via Sesion open-window JOIN (DA-F12.1-10)
...
### REQ-OPS-187 — `<MiTurnoPanel />` mount and `Cerrar-turno` button (DA-F12.1-4, DA-F12.1-5)
...
### REQ-OPS-188 — `useMiTurno` SWR hook (DA-F12.1-3)
...
### REQ-OPS-189 — FE Zod schema matches BE Pydantic (DA-F12.1-1, DA-F12.1-9)
...
### REQ-OPS-190 — Test coverage contract (DA-F12.1-8)
...
- THEN ≥10 scenarios pass: happy path, zero state, cross-branch 403, closed-session window, `medio_pago` breakdown, missing session 404, multi-pago mix, plus ≥4 negative paths.
...
- **ABBC-F12.x** — backend JOIN refactor (out of F12.1 scope): if a future HU replaces the temporal JOIN with a `uuid_sesion` FK on `[L-E]` / `[A]` tables, the ER.mmd canon must be amended first (KD-3 invariant — no migration may bypass DB-layer immutability without ADR).
- **v2 WebSocket sync** — `useMiTurno` SWR `refreshInterval` will be removed when WS lands; same as F11.x forward hook.
...
## F11.x Spec-Delta Appendix (carry-overs reconciled)

The following items remain authoritative across Fase 11/12 boundary:

- **R-CARRY-1** — derived `estado` thresholds in `apiStatusStore`. F12.1 spec does NOT modify `apiStatusStore` (F11.1 REQ-OPS-173 contract remains intact). Forward hook (F11.2 RR) `consecutiveFailures >= 3` suppression unchanged.
- **REQ-OPS-173** — `LOCAL_API_DOWN_THRESHOLD = 3` constant. F12.1 spec uses a separate `miTurno.*` i18n namespace (no collision). REQ-OPS-173 contract unchanged.
- **F11.2 carryover REQ-OPS-177..183** — unchanged. F12.1 deltas the `operations` domain with REQ-OPS-184..190; F11.2 entries remain authoritative.
