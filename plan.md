...
- **HU-F8.4-T5** (Ajuste 2026-09-25, Flujo 4 de la directiva de identificación persona/empresa — lado LECTURA, cierra la directiva completa): `FacturaDisplayCliente` (`schemas/facturacion.py`) gana `tipo_identificador: Literal["NIT","CC","CE","pasaporte"] | None` y renombra `nit`→`numero_identificacion` (consistente con el nombre canónico usado en `schemas/clientes.py` y el resto del backend); `_factura_display.py::build_display_factura` mapea `tipo_identificador` real desde `Clientes` en vez de asumir NIT. Frontend: `facturaApi.ts` (`clienteSchema`) espeja el rename + el nuevo campo; `<FacturaDisplayModal>` muestra la etiqueta del tipo de documento (NIT / Cédula de ciudadanía / Cédula de extranjería / Pasaporte, reusando las claves i18n de `<ClienteIdentificacionFields>`) y el sufijo `-DV` solo cuando `tipo_identificador==='NIT'` (antes se mostraba para cualquier tipo). Con esto, persona natural/empresa queda soportado de punta a punta (captura + visualización) en todos los flujos de facturación de `apps/electron-sucursal`.
...
- **HU-F9.1-T5** (Ajuste 2026-09-25, Flujo 2 de la directiva de identificación persona/empresa — depende de HU-F8.1-T6): paso 1 de `Venta.tsx` reemplaza el input NIT-only por `<ClienteIdentificacionFields>` (componente compartido con `PagoModal`, `src/features/facturacion/components/ClienteIdentificacionFields.tsx`); armado del payload centralizado en `src/features/suscripciones/lib/clienteVentaPayload.ts` (`buildClienteVentaPayload`, función pura — ya no hardcodea `tipo_identificador:'NIT'`); `ventaSuscripcionApi.ts` cambia `tipo_identificador: z.literal('NIT')` por el enum completo. Backend: `uuid_tipo_persona` se resuelve server-side desde `tipo_identificador` vía `repo/tipo_persona.py::resolve_uuid_tipo_persona` (NIT→jurídica, CC/CE/pasaporte→natural), invocado desde `repo/venta_suscripcion.py::buscar_cliente_por_uuid_o_crear_nuevo` — reutilizable por futuros call-sites (ej. cuando `factura.py` empiece a auto-crear clientes) sin duplicar el mapeo.
  ...
  - **Hallazgo de diseño preexistente — RESUELTO (2026-09-25)**: `buildVentaPayload` (`Venta.tsx`) arma el campo `cliente` del `POST /clientes/venta-suscripcion` **exclusivamente** desde `state.cliente` (paso 1) — los campos NIT/DV/tipo de documento/nombre/apellido/email que `<PagoModal>` muestra en el paso 5 (vía `clientePrefill`) eran puramente decorativos, y su validación de DV módulo 11 sí bloqueaba el submit sobre un valor que después se descartaba. **Decisión del operador**: los campos de identificación en el paso 5 quedan fijados desde el paso 1, en modo solo lectura — un solo cliente es titular de la suscripción y de la FE, sin cambios de modelo ni de contrato backend. **Implementación**: `<PagoModal>` gana la prop `identificacionReadonly?: boolean` (`src/features/facturacion/components/PagoModal.tsx`) — deshabilita los inputs/selects del bloque de identificación y, al activarse, el `superRefine(validarBloqueFe)` del schema Zod se salta por completo (la validación cruzada de cliente no aplica a un bloque no editable cuyo valor real viene del paso 1). `<Venta>` paso 5 pasa `identificacionReadonly` al montar `<PagoModal>`; `<PagoSheet>`/`<ReimprimirTiquete>` no la pasan (default `false`, comportamiento sin cambios — ahí el bloque sigue siendo la fuente de verdad).
